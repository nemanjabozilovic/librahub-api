...
public class OrderRepository(OrdersDbContext context) : IOrderRepository
{
    ...
    public async Task<OrderStatisticsResult> GetStatisticsAsync(DateTime last30Days, DateTime last7Days, DateTime now, CancellationToken cancellationToken = default)
    {
        ...
        var paid = await context.Orders.CountAsync(o => o.Status == OrderStatus.Paid, cancellationToken);
        var pending = await context.Orders.CountAsync(o => o.Status == OrderStatus.PaymentPending, cancellationToken);
        var cancelled = await context.Orders.CountAsync(o => o.Status == OrderStatus.Cancelled, cancellationToken);
        var refunded = await context.Orders.CountAsync(o => o.Status == OrderStatus.Refunded, cancellationToken);

        var totalRevenue = await context.Orders
            .Where(o => o.Status == OrderStatus.Paid)
            .SumAsync(o => o.Total.Amount, cancellationToken);

        var last30DaysRevenue = await context.Orders
            .Where(o => o.Status == OrderStatus.Paid && o.CreatedAt >= last30Days && o.CreatedAt <= now)
            .SumAsync(o => o.Total.Amount, cancellationToken);

        var last7DaysRevenue = await context.Orders
            .Where(o => o.Status == OrderStatus.Paid && o.CreatedAt >= last7Days && o.CreatedAt <= now)
            .SumAsync(o => o.Total.Amount, cancellationToken);

        var last30DaysCount = await context.Orders
            .CountAsync(o => o.Status == OrderStatus.Paid && o.CreatedAt >= last30Days && o.CreatedAt <= now, cancellationToken);

        var last7DaysCount = await context.Orders
