...
public class OrderPaidConsumerTests
{
    ...
    private readonly Mock<ILogger<OrderPaidConsumer>> _logger = new();

    private readonly Guid _userId = Guid.NewGuid();
    private readonly Guid _orderId = Guid.NewGuid();

    private OrderPaidConsumer CreateConsumer()
    {
        _unitOfWork
    ...
    [Fact]
    public async Task HandleAsync_WhenAlreadyProcessed_SkipsAndDoesNotMarkAgain()
    {
    ...
    [Fact]
    public async Task HandleAsync_WhenInactiveUser_SkipsAndMarksProcessed()
    {
        _inboxRepository.Setup(r => r.IsProcessedAsync(It.IsAny<string>(), It.IsAny<CancellationToken>())).ReturnsAsync(false);
        _settingsRepository.Setup(r => r.GetByUserIdAsync(_userId, It.IsAny<CancellationToken>())).ReturnsAsync(Settings(isActive: false));
        var consumer = CreateConsumer();
        ...
        _notificationRepository.Verify(r => r.AddAsync(It.IsAny<Notification>(), It.IsAny<CancellationToken>()), Times.Never);
        _notificationSender.Verify(s => s.SendInAppAsync(It.IsAny<Guid>(), It.IsAny<string>(), It.IsAny<string>(), It.IsAny<CancellationToken>()), Times.Never);
    }

    [Fact]
    public async Task HandleAsync_WhenStaffUser_Skips()
    {
        _inboxRepository.Setup(r => r.IsProcessedAsync(It.IsAny<string>(), It.IsAny<CancellationToken>())).ReturnsAsync(false);
    ...
    public async Task HandleAsync_WhenInAppEnabled_CreatesNotificationAndSendsInApp()
    {
        ...
        _notificationRepository.Verify(r => r.AddAsync(It.Is<Notification>(n => n.UserId == _userId && n.Type == NotificationType.OrderPaid), It.IsAny<CancellationToken>()), Times.Once);
    ...
    [Fact]
    public async Task HandleAsync_WhenInAppDisabled_DoesNotCreateNotification()
    {
        _inboxRepository.Setup(r => r.IsProcessedAsync(It.IsAny<string>(), It.IsAny<CancellationToken>())).ReturnsAsync(false);
        _settingsRepository.Setup(r => r.GetByUserIdAsync(_userId, It.IsAny<CancellationToken>())).ReturnsAsync(Settings(inAppEnabled: false));
        var consumer = CreateConsumer();
    ...
    public async Task HandleAsync_WhenEmailEnabledAndUserActive_SendsEmail()
    {
        ...
        _identityClient
            ...
            .ReturnsAsync(Result.Success(new UserInfo { Id = _userId, Email = "user@example.com", FullName = "John Doe", IsActive = true }));
        var consumer = CreateConsumer();

        await consumer.HandleAsync(Event(), CancellationToken.None);

        _notificationSender.Verify(
            s => s.SendEmailWithTemplateAsync("user@example.com", It.IsAny<string>(), "ORDER_PAID", It.IsAny<object>(), It.IsAny<CancellationToken>()),
            Times.Once);
    }

    [Fact]
    public async Task HandleAsync_WhenEmailEnabledButUserInfoInactive_DoesNotSendEmail()
    {
        _inboxRepository.Setup(r => r.IsProcessedAsync(It.IsAny<string>(), It.IsAny<CancellationToken>())).ReturnsAsync(false);
        _settingsRepository.Setup(r => r.GetByUserIdAsync(_userId, It.IsAny<CancellationToken>())).ReturnsAsync(Settings(emailEnabled: true));
        _identityClient
            .Setup(c => c.GetUserInfoAsync(_userId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result.Success(new UserInfo { Id = _userId, Email = "user@example.com", IsActive = false }));
        var consumer = CreateConsumer();
